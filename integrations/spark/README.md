...
Spark notebook on localhost:8888. On startup, the notebook container will show a list of URLs including
an access token, such as
```bash
...
gradle shadowJar
```
then reference it in the Jupyter notebook with
```python
from pyspark.sql import SparkSession
...
t1 = DataProcPySparkOperator(
    ...
    dataproc_pyspark_properties={
      ...
      "openlineage.url": "{marquez_url}/api/v1/namespaces/{marquez_namespace}/jobs/dump_orders_to_gcs/runs/{{{{task_run_id(run_id, task)}}}}?api_key={api_key}"
    },
    dag=dag)
```
...
t1 = DataProcPySparkOperator(
    ...
    dataproc_pyspark_properties={
      'spark.driver.extraJavaOptions':
        f"-javaagent:{jar}={marquez_url}/api/v1/namespaces/{marquez_namespace}/jobs/dump_orders_to_gcs/runs/{{{{task_run_id(run_id, task)}}}}?api_key={api_key}"
    files=files,
    dag=dag)
```
